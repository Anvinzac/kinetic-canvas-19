...
BEGIN
  ...
  FOR attempt_index IN 0..11 LOOP
    ...
    clue := CASE candidate_index
      ...
      WHEN 4 THEN ARRAY[
        'Sự bình tĩnh khi mọi thứ đang rối.',
        'Từ này bắt đầu bằng chữ C.',
        'Cả từ gồm 9 chữ cái.',
        'Bạn có cảm nhận được nghĩa không?',
        'Composure'
      ]::TEXT[]
      WHEN 5 THEN ARRAY[
        'Một ý nghĩ hiện ra rất nhanh rồi biến mất.',
        'Từ này bắt đầu bằng chữ G.',
        'Cả từ gồm 7 chữ cái.',
        'Đừng để nó trôi qua nhé.',
        'Glimpse'
      ]::TEXT[]
      WHEN 6 THEN ARRAY[
        'Ánh sáng dịu xuất hiện ngay trước bình minh.',
        'Từ này bắt đầu bằng chữ D.',
        'Cả từ gồm 4 chữ cái.',
        'Trang cuối sẽ bật mí.',
        'Dawn'
      ]::TEXT[]
      WHEN 7 THEN ARRAY[
        'Sự tò mò khiến bạn muốn tìm hiểu thêm.',
        'Từ này bắt đầu bằng chữ C.',
        'Cả từ gồm 9 chữ cái.',
        'Một từ rất hợp với người học.',
        'Curiosity'
      ]::TEXT[]
      WHEN 8 THEN ARRAY[
        'Một thói quen nhỏ lặp lại mỗi ngày.',
        'Từ này bắt đầu bằng chữ R.',
        'Cả từ gồm 7 chữ cái.',
        'Bạn gặp nó mỗi sáng.',
        'Routine'
      ]::TEXT[]
      WHEN 9 THEN ARRAY[
        'Cảm giác nhẹ nhõm sau khi vấn đề được giải quyết.',
        'Từ này bắt đầu bằng chữ R.',
        'Cả từ gồm 6 chữ cái.',
        'Thở ra một chút nhé.',
        'Relief'
      ...
      WHEN 10 THEN ARRAY[
        'Một chi tiết nhỏ làm mọi thứ trở nên rõ hơn.',
        'Từ này bắt đầu bằng chữ C.',
        'Cả từ gồm 4 chữ cái.',
        'Nó giúp bạn hiểu bức tranh lớn.',
        'Clue'
      ]::TEXT[]
      WHEN 11 THEN ARRAY[
        'Sự bền bỉ khi bạn tiếp tục dù chưa dễ dàng.',
        'Từ này bắt đầu bằng chữ P.',
        'Cả từ gồm 11 chữ cái.',
        'Từ này hơi dài, nhưng đáng nhớ.',
        'Persistence'
      ]::TEXT[]
      ELSE ARRAY[
        'Khoảnh khắc một ý tưởng bỗng sáng lên.',
        'Từ này bắt đầu bằng chữ I.',
        'Cả từ gồm 7 chữ cái.',
        'Nó thường đến rất bất ngờ.',
        'Insight'
      ]::TEXT[]
    END;
    ...
    IF NOT EXISTS (
      SELECT 1
      FROM public.posts
      WHERE author_id = agent.profile_id
        AND canvas_html ILIKE '%' || reveal_word || '%'
    ) THEN
      EXIT;
    END IF;
