...
-- External feeder services POST items here; scheduled bot functions claim one
-- unused item and publish it.
...
GRANT SELECT ON public.agent_content_sources TO service_role;
GRANT SELECT ON public.agent_content_items TO service_role;
GRANT INSERT, UPDATE, DELETE ON public.agent_content_sources TO service_role;
GRANT INSERT, UPDATE, DELETE ON public.agent_content_items TO service_role;
...
DROP POLICY IF EXISTS "agent_content_sources_select_authenticated" ON public.agent_content_sources;
DROP POLICY IF EXISTS "agent_content_sources_select_service" ON public.agent_content_sources;
CREATE POLICY "agent_content_sources_select_service"
  ON public.agent_content_sources FOR SELECT TO service_role
  USING (true);

DROP POLICY IF EXISTS "agent_content_items_select_authenticated" ON public.agent_content_items;
DROP POLICY IF EXISTS "agent_content_items_select_service" ON public.agent_content_items;
CREATE POLICY "agent_content_items_select_service"
  ON public.agent_content_items FOR SELECT TO service_role
  USING (true);
...
DECLARE
  item_id UUID;
BEGIN
  INSERT INTO public.agent_content_items (
    source_key,
    content_key,
    payload,
    available_at,
  ...
  SET
    ...
    status = CASE
      WHEN public.agent_content_items.status = 'used' THEN public.agent_content_items.status
      ELSE 'ready'
    END,
    updated_at = now()
...
REVOKE ALL ON FUNCTION public.enqueue_agent_content_item(TEXT, TEXT, JSONB, TIMESTAMPTZ)
FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.enqueue_agent_content_item(TEXT, TEXT, JSONB, TIMESTAMPTZ)
TO service_role;
...
SELECT public.enqueue_agent_content_item(
  ...
  jsonb_build_object(
    'word', item.word,
    'vi_definition', item.vi_definition,
    'hints', item.hints,
