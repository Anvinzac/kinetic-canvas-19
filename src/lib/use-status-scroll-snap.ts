...
export function useStatusScrollSnap<T extends HTMLElement = HTMLElement>(itemCount: number) {
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (event: TouchEvent) => {
      ...
      const maxTop = getMaxIndex() * getPageHeight();
      container.dataset.snapLock = "true";
    ...
    const handleTouchEnd = () => {
      ...
      if (travel > pageHeight * SWIPE_PAGE_THRESHOLD || velocity > SWIPE_VELOCITY_THRESHOLD) {
        targetIndex += 1;
      } else if (
  ...
  }, [getMaxIndex, getNearestIndex, getPageHeight, scheduleSnap, snapToIndex]);
